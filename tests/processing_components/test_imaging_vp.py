...
class TestImagingVP(unittest.TestCase):
        ...
        def find_vp_renormalised(model, renormalise=False):
            if telescope == "MID_FEKO_B2":
                ...
                vp = create_vp_generic(model, diameter=15.0, polarisation_frame=PolarisationFrame("linear"),
                                       no_cross_pol=False, cross_pol=[-0.003 + 0.002j, +0.002 - 0.006j])
            else:
    ...
    def _checkcomponents(self, dirty, fluxthreshold=0.6, positionthreshold=0.1):
        ...
        for comp in comps:
            ...
            assert err < fluxthreshold, "{} Component has significant error {}".format(self.telescope, comp.flux - ocomp.flux)
    ...
    def _invert_base(self, fluxthreshold=0.6, positionthreshold=1.0, check_components=True,
        ...
        if self.persist:
            ...
            export_image_to_fits(dirty_fft,
                                 '%s/test_imaging_vp_%s_mid_%s_dirty_fft.fits' % (self.dir, self.telescope, name))
    ...
    def test_invert_vp(self):
        for telescope in ["MID_PERFECT", "MID_IMPERFECT", "MID_FEKO_B2"]:
            self.telescope = telescope
            self.actualSetUp(zerow=True, block=True, telescope=telescope)
            self._invert_base(name='invert_vp', positionthreshold=2.0, check_components=True, fluxthreshold=3.5)
    ...
    def test_invert_vp_weights(self):
        ...
        for telescope in ["MID_PERFECT", "MID_IMPERFECT", "MID_FEKO_B2"]:
            self.telescope = telescope
            self.actualSetUp(zerow=True, block=True, telescope=telescope)
            self._invert_base(name='invert_vp_weights', positionthreshold=2.0, check_components=False,
                              grid_weights=True)
            
            for pol in range(4):
                pol_name = self.dirty[0].polarisation_frame.names[pol]
                maxval = numpy.max(self.dirty[0].data[:, pol]).real
                if abs(maxval - 1.0) > 5e-2:
                    print(telescope, pol_name, maxval)
                    maximum_ok = False
            ...
            for pol in range(4):
                ...
                if minval < -5e-2:
                    print(telescope, pol_name, minval)
                    minimum_ok = False
