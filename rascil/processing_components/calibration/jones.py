...
def apply_jones_einsum(ej, cfs, inverse=False, min_det=1e-6):
    """ Apply Jones matrix (or inverse)
    ...
    if inverse:
        if numpy.abs(numpy.linalg.det(ej)) > min_det:
            inv_ej = numpy.linalg.inv(ej)
            inv_cej = numpy.conjugate(inv_ej).T
            product = numpy.einsum("ij,jk->ik", inv_ej, cfs)
            product = numpy.einsum("ij,jk->ik", product, inv_cej)
            return product
        else:
            return 0.0 * cfs
    else:
        cej = numpy.conjugate(ej).T
        product = numpy.einsum("ij,jk->ik", ej, cfs)
        product = numpy.einsum("ij,jk->ik", product, cej)
        return product
